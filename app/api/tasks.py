...
async def get_task_status(task_id: str):
    ...
    try:
        ...
        if result.state == "SUCCESS":
            ...
            response_data["error"] = (
                str(result.info) if result.info else "Task is being retried"
            )
        ...
        logger.error(
            f"Error fetching task status for {task_id}: {str(e)}", exc_info=True
        )
        raise HTTPException(
...
async def retry_task(task_id: str):
    ...
    try:
        ...
        if not task_func:
            ...
            raise HTTPException(
                status_code=400, detail=f"Task function {task_name} not found"
            )
...
async def list_task_errors(
    ...
    """
    logger.info(
        f"Listing task errors with severity={severity}, limit={limit}, offset={offset}"
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error listing task errors: {str(e)}"
        )
...
async def get_task_error(error_id: int):
    ...
    try:
        ...
        if not error:
            ...
            raise HTTPException(
                status_code=404, detail=f"Task error not found: {error_id}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching task error: {str(e)}"
        )
...
async def start_metadata_sync_batch(request: BatchOperationCreate):
    ...
    """
    logger.info(
        f"Starting metadata sync batch with {len(request.media_ids or [])} items"
    )
    ...
    try:
        ...
        if not request.media_ids or len(request.media_ids) == 0:
            raise HTTPException(
                status_code=400, detail="media_ids list cannot be empty"
            )

        if not request.media_type or request.media_type not in ["movie", "tv_show"]:
            raise HTTPException(
                status_code=400, detail="media_type must be 'movie' or 'tv_show'"
            )
        ...
        try:
            ...
            # Start async task
            task = bulk_metadata_sync_task.delay(
                batch_op.id, request.media_ids, request.media_type
            )

            logger.info(
                f"Started metadata sync batch {batch_op.id} with task {task.id}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error starting metadata sync batch: {str(e)}"
        )
...
async def start_file_import_batch(request: BatchOperationCreate):
    ...
    """
    logger.info(
        f"Starting file import batch with {len(request.file_paths or [])} files"
    )
    ...
    try:
        ...
        if not request.file_paths or len(request.file_paths) == 0:
            raise HTTPException(
                status_code=400, detail="file_paths list cannot be empty"
            )
        ...
        try:
            ...
            # Start async task
            task = bulk_file_import_task.delay(
                batch_op.id, request.file_paths, request.media_type
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error starting file import batch: {str(e)}"
        )
...
async def get_batch_operation(batch_id: int):
    ...
    try:
        ...
        logger.error(
            f"Error fetching batch operation {batch_id}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Error fetching batch operation: {str(e)}"
        )
...
async def list_batch_operations(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error listing batch operations: {str(e)}"
        )
...
async def cancel_batch_operation(batch_id: int):
    ...
    try:
        ...
        logger.error(
            f"Error cancelling batch operation {batch_id}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Error cancelling batch operation: {str(e)}"
        )
...
async def get_batch_progress(batch_id: int):
    ...
    try:
        ...
        logger.error(
            f"Error fetching batch progress {batch_id}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Error fetching batch progress: {str(e)}"
        )
