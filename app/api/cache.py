...
async def get_cache_stats(
    ...
    try:
        ...
        logger.info(
            f"Retrieved cache statistics: {stats['total_entries']} total entries"
        )
        return stats
...
async def delete_expired_cache(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete expired cache entries"
        )
...
async def invalidate_cache_by_type(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to invalidate cache entries"
        )
...
async def get_redis_cache_stats():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve Redis cache statistics"
        )
...
async def warmup_redis_cache(
    ...
    """
    try:
        from app.services_impl import MovieService, TVShowService
...
async def get_slow_queries(
    limit: int = Query(
        10, ge=1, le=100, description="Number of slow queries to return"
    ),
):
    """
