...
class TaskErrorHandler:
    ...
    def notify_failure(
        ...
        try:
            # Log notification
            log_level = (
                logging.ERROR if severity == TaskErrorHandler.SEVERITY_CRITICAL else logging.WARNING
            )
            logger.log(
                log_level,
    ...
    def _store_error_in_db(
        ...
        try:
            ...
            existing_error = db.query(TaskError).filter(TaskError.task_id == task_id).first()
