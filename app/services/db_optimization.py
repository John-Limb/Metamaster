    ...
    def get_pool_stats(self, pool: Pool) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.pool_stats_history:
                checked_out_values = [s["checked_out"] for s in self.pool_stats_history if s["checked_out"] is not None]
                overflow_values = [s["overflow"] for s in self.pool_stats_history if s["overflow"] is not None]
                
                avg_checked_out = sum(checked_out_values) / len(checked_out_values) if checked_out_values else 0
