...
class OMDBService:
    ...
    @staticmethod
    def _set_cache(db: Session, cache_key: str, response_data: Dict[str, Any], ttl_seconds: int) -> None:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if e.response.status_code == 429:  # Rate limited
                        ...
                        logger.warning(f"Rate limited by OMDB. Retrying in {delay}s (attempt {attempt + 1}/{max_retries})")
                        await asyncio.sleep(delay)
                    elif e.response.status_code >= 500:  # Server error
                        delay = base_delay * (2 ** attempt)
                        logger.warning(f"OMDB server error ({e.response.status_code}). Retrying in {delay}s (attempt {attempt + 1}/{max_retries})")
                        await asyncio.sleep(delay)
                    else:
                    ...
                    delay = base_delay * (2 ** attempt)
                    logger.warning(f"OMDB request error: {e}. Retrying in {delay}s (attempt {attempt + 1}/{max_retries})")
                    await asyncio.sleep(delay)
    ...
    def parse_omdb_response(omdb_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if "Search" in omdb_data:
                ...
                return {"search_results": results, "total_results": int(omdb_data.get("totalResults", 0))}
            ...
            if "imdbID" in omdb_data:
                ...
                return {
                    ...
                    "year": int(omdb_data.get("Year", 0)) if omdb_data.get("Year", "").isdigit() else None,
...
class TVDBService:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if data.get("status") == "success" and "data" in data:
                    ...
                    logger.error(f"TVDB authentication failed: {data.get('message', 'Unknown error')}")
                    return None
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            for attempt in range(max_retries):
                try:
                    ...
                    if e.response.status_code == 429:  # Rate limited
                        delay = base_delay * (2 ** attempt)
                        logger.warning(f"Rate limited by TVDB. Retrying in {delay}s (attempt {attempt + 1}/{max_retries})")
                        await asyncio.sleep(delay)
                        ...
                        delay = base_delay * (2 ** attempt)
                        logger.warning(f"TVDB server error ({e.response.status_code}). Retrying in {delay}s (attempt {attempt + 1}/{max_retries})")
                        await asyncio.sleep(delay)
                    else:
                    ...
                    delay = base_delay * (2 ** attempt)
                    logger.warning(f"TVDB request error: {e}. Retrying in {delay}s (attempt {attempt + 1}/{max_retries})")
                    await asyncio.sleep(delay)
    ...
    @staticmethod
    def parse_tvdb_search_response(tvdb_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def parse_tvdb_series_response(tvdb_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def parse_tvdb_seasons_response(tvdb_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def parse_tvdb_episodes_response(tvdb_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
...
class CacheService:
    ...
    def invalidate_by_type(db: Session, api_type: str) -> int:
        ...
        """
        try:
            entries = db.query(APICache).filter(
                APICache.api_type == api_type
            ).all()
    ...
    def invalidate_by_pattern(db: Session, pattern: str) -> int:
        ...
        try:
            ...
            entries = db.query(APICache).filter(
                APICache.query_key.like(sql_pattern)
            ).all()
    ...
    def get_cache_stats(db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            expired_entries = db.query(func.count(APICache.id)).filter(
                APICache.expires_at <= datetime.utcnow()
            ).scalar() or 0
            ...
            type_breakdown = db.query(
                APICache.api_type,
                func.count(APICache.id).label("count")
            ).group_by(APICache.api_type).all()

            type_stats = {api_type: count for api_type, count in type_breakdown}

            # Calculate total cache size in bytes
            total_size = db.query(
                func.sum(func.length(APICache.response_data))
            ).scalar() or 0
            ...
            logger.info(f"Cache stats: {total_entries} total, {active_entries} active, {total_size} bytes")
            return stats
    ...
    def bulk_delete_cache(db: Session, cache_keys: list) -> int:
        ...
        try:
            entries = db.query(APICache).filter(
                APICache.query_key.in_(cache_keys)
            ).all()
