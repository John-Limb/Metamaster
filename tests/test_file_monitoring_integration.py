...
class TestFileDetection:
    ...
    def test_ignore_non_media_files(self, temp_media_dir):
        ...
        media_files = [
            f for f in Path(temp_media_dir).glob("*")
...
class TestConcurrentFileProcessing:
    ...
    def test_process_multiple_files_sequentially(self, db_session):
        ...
        completed = db_session.query(FileQueue).filter(
            FileQueue.status == "completed"
        ).all()
    ...
    def test_process_mixed_media_types(self, db_session):
        ...
        # Verify counts
        movies = db_session.query(FileQueue).filter(
            FileQueue.media_type == "movie"
        ).all()
        shows = db_session.query(FileQueue).filter(
