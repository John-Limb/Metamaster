...
class TestTaskErrorHandling:
    ...
    def test_task_error_persistence(self, db_session):
        ...
        retrieved = db_session.query(TaskError).filter(
            TaskError.task_id == "task-123"
        ).first()
...
class TestTaskResultBackend:
    ...
    def test_task_result_expiration(self, db_session):
        ...
        recent = db_session.query(TaskError).filter(
            TaskError.created_at > datetime.utcnow() - timedelta(days=7)
        ).all()
