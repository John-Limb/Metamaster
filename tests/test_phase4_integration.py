...
class TestScenario1CompleteFileProcessingPipeline:
    ...
    @pytest.mark.asyncio
    async def test_complete_pipeline_movie_file(self, temp_watch_dir, queue_manager, pattern_service):
    ...
    @pytest.mark.asyncio
    async def test_complete_pipeline_tv_show_file(self, temp_watch_dir, queue_manager, pattern_service):
...
class TestScenario2MovieFileDetectionAndProcessing:
    ...
    @pytest.mark.asyncio
    async def test_movie_detection_and_processing(self, temp_watch_dir, queue_manager, pattern_service):
...
class TestScenario3TVShowFileDetectionAndProcessing:
    ...
    @pytest.mark.asyncio
    async def test_tv_show_detection_and_processing(self, temp_watch_dir, queue_manager, pattern_service):
    ...
    def test_tv_show_classification_accuracy(self, pattern_service):
        ...
        test_cases = [
            ...
            ("Stranger Things Season 1 Episode 1.mov", "tv_show", "Stranger Things", 1, 1),
        ]
        
        for filename, expected_type, expected_show, expected_season, expected_episode in test_cases:
            classification = pattern_service.classify_file(filename)
            assert classification["type"] == expected_type
...
class TestScenario4MixedContentProcessing:
    ...
    async def test_mixed_content_processing(self, temp_watch_dir, queue_manager, pattern_service):
        ...
        movies = queue_manager.session.query(FileQueue).filter(
            FileQueue.media_type == "movie"
        ).count()
        tv_shows = queue_manager.session.query(FileQueue).filter(
            FileQueue.media_type == "tv_show"
        ).count()
