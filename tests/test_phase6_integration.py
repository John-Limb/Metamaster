...
from app.services.db_optimization import (
    QueryPerformanceTracker, DatabaseOptimizationService
)
...
class TestRedisCachingWithSearchFilters:
    ...
    def test_cache_search_results(self, test_db, cache_service, sample_movies):
        ...
        search_results = [{"id": m.id, "title": m.title} for m in sample_movies if "Drama" in json.loads(m.genres)]
    ...
    def test_cache_hit_on_repeated_search(self, test_db, cache_service, sample_movies):
        ...
        # Set cache
        search_data = {"results": [m.title for m in sample_movies if "Action" in json.loads(m.genres)]}
