...
class TestTVShowImportWorkflow:
    ...
    def test_tv_show_import_workflow_complete(self, db_session, temp_media_dir):
        ...
        season_1 = db_session.query(Season).filter(
        ...
        for episode in episodes:
            file_path = os.path.join(temp_media_dir, f"Breaking.Bad.S01E{episode.episode_number:02d}.mkv")
...
class TestSearchAndFilterWorkflow:
    ...
    def test_search_workflow_filter_combinations(self, db_session):
        ...
        scifi_recent = db_session.query(Movie).filter(
            Movie.year >= 2000,
            Movie.genres.like('%Sci-Fi%')
        ).all()
...
class TestDataConsistencyWorkflow:
    ...
    def test_data_consistency_movie_cascade_delete(self, db_session):
        ...
        for i in range(3):
            file = MovieFile(
                movie_id=movie.id,
                file_path=f"/path/to/file_{i}.mkv"
            )
            db_session.add(file)
        db_session.commit()
        
        movie_id = movie.id
        file_count_before = db_session.query(MovieFile).filter(MovieFile.movie_id == movie_id).count()
        ...
        file_count_after = db_session.query(MovieFile).filter(MovieFile.movie_id == movie_id).count()
    ...
    def test_data_consistency_tv_show_cascade_delete(self, db_session):
        ...
        episode_count_before = db_session.query(Episode).join(Season).filter(Season.show_id == show_id).count()
        ...
        episode_count_after = db_session.query(Episode).join(Season).filter(Season.show_id == show_id).count()
