...
class PerformanceMetrics:
    ...
    def __post_init__(self):
        if self.iterations > 0:
            self.throughput = self.iterations / self.execution_time if self.execution_time > 0 else 0
...
class ConcurrencyTester:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (len(self.operation_times) / (len(self.operation_times) + len(self.errors)) * 100) if (len(self.operation_times) + len(self.errors)) > 0 else 0,
        }
