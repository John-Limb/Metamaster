...
from app.models import Base, APICache
...
class TestCacheServiceKeyGeneration:
    ...
    def test_generate_cache_key_basic(self):
        ...
        key = CacheService.generate_cache_key("omdb", "search", {"title": "Inception"})
        assert key == "omdb:search:title=Inception"
    ...
    def test_generate_cache_key_multiple_params(self):
        ...
        assert key.startswith("omdb:search:")

    def test_generate_cache_key_tvdb(self):
        """Test cache key generation for TVDB"""
        key = CacheService.generate_cache_key("tvdb", "series", {"tvdb_id": "121361"})
        assert key == "tvdb:series:tvdb_id=121361"
...
class TestCacheServiceSetGet:
    ...
    def test_set_and_get_cache(self, db):
        ...
        # Set cache
        result = CacheService.set_cache(db, cache_key, test_data, api_type="omdb", ttl_seconds=3600)
    ...
    def test_set_cache_updates_existing(self, db):
        ...
        # Set initial cache
        CacheService.set_cache(db, cache_key, old_data, api_type="omdb")

        # Update cache
        CacheService.set_cache(db, cache_key, new_data, api_type="omdb")
...
class TestCacheServiceDelete:
    ...
    def test_delete_cache(self, db):
        ...
        # Set cache
        CacheService.set_cache(db, cache_key, test_data, api_type="omdb")
    ...
    def test_bulk_delete_cache(self, db):
        ...
        for key in keys:
            CacheService.set_cache(db, key, test_data, api_type="omdb")
...
class TestCacheServiceInvalidation:
    ...
    def test_invalidate_by_type(self, db):
        ...
        CacheService.set_cache(db, "omdb:key:1", {"data": "omdb1"}, api_type="omdb")
        CacheService.set_cache(db, "omdb:key:2", {"data": "omdb2"}, api_type="omdb")

        # Create TVDB cache entries
        CacheService.set_cache(db, "tvdb:key:1", {"data": "tvdb1"}, api_type="tvdb")

        # Invalidate OMDB
        deleted_count = CacheService.invalidate_by_type(db, "omdb")
        assert deleted_count == 2

        # Verify OMDB entries are gone
        assert CacheService.get_cache(db, "omdb:key:1") is None
        assert CacheService.get_cache(db, "omdb:key:2") is None

        # Verify TVDB entry still exists
        assert CacheService.get_cache(db, "tvdb:key:1") is not None
    ...
    def test_invalidate_by_pattern(self, db):
        ...
        assert CacheService.get_cache(db, "omdb:search:inception") is None
        assert CacheService.get_cache(db, "omdb:search:matrix") is None

        # Verify details entry still exists
        assert CacheService.get_cache(db, "omdb:details:tt0111161") is not None
...
class TestCacheServiceStatistics:
    ...
    def test_get_cache_stats_with_entries(self, db):
        ...
        # Create entries
        CacheService.set_cache(db, "omdb:key:1", {"data": "test1"}, api_type="omdb")
        CacheService.set_cache(db, "omdb:key:2", {"data": "test2"}, api_type="omdb")
        CacheService.set_cache(db, "tvdb:key:1", {"data": "test3"}, api_type="tvdb")
        ...
        assert stats["expired_entries"] == 0
        assert stats["by_api_type"]["omdb"] == 2
        assert stats["by_api_type"]["tvdb"] == 1
        assert stats["total_size_bytes"] > 0

    def test_get_cache_stats_with_expired(self, db):
        """Test cache stats includes expired entries"""
        # Create active entry
        CacheService.set_cache(db, "omdb:key:1", {"data": "active"}, api_type="omdb")
...
class TestCacheServiceBulkOperations:
    ...
    def test_clear_all_cache(self, db):
        ...
        for i in range(5):
            CacheService.set_cache(db, f"test:key:{i}", {"data": f"test{i}"}, api_type="omdb")
    ...
    def test_get_cache_by_type(self, db):
        ...
        # Create OMDB entries
        for i in range(3):
            CacheService.set_cache(db, f"omdb:key:{i}", {"data": f"omdb{i}"}, api_type="omdb")

        # Create TVDB entries
        for i in range(2):
            CacheService.set_cache(db, f"tvdb:key:{i}", {"data": f"tvdb{i}"}, api_type="tvdb")

        # Get OMDB entries
        entries, total = CacheService.get_cache_by_type(db, "omdb")
        assert total == 3
        assert len(entries) == 3

        # Get TVDB entries
        entries, total = CacheService.get_cache_by_type(db, "tvdb")
        assert total == 2
    ...
    def test_get_cache_by_type_pagination(self, db):
        ...
        # Get first page
        entries, total = CacheService.get_cache_by_type(db, "omdb", limit=5, offset=0)
        assert total == 10
        assert len(entries) == 5

        # Get second page
        entries, total = CacheService.get_cache_by_type(db, "omdb", limit=5, offset=5)
        assert total == 10
        assert len(entries) == 5
